...
public:
	...
	virtual IloExtractableI* makeClone(IloEnvI* env) const {
		...
		if (cond->isType(IloConstConstraintI::GetTypeInfo())){
			IloConstConstraintI* c = (IloConstConstraintI*)cond;
			if (c->isTrue()) return (BaseClass*)env->getClone(_then);
			else return (BaseClass*)env->getClone(_else);
		}
...
IloIntExprArg IloAtoi(const IloSymbolExprArg arg);
IloSymbolExprArg IloItoa(const IloIntExprArg arg);

class IloAtoiI : public IloIntExprI {
	ILOEXTRDECL
		IloSymbolExprI* _arg;
	virtual void visitSubExtractables(IloExtractableVisitor* v);
public:
	IloAtoiI(IloEnvI* env, IloSymbolExprI* arg) :
		IloIntExprI(env), _arg((IloSymbolExprI*)arg->lockExpr()) {}
	virtual ~IloAtoiI();
	void display(ILOSTD(ostream)& out) const;
	virtual IloNum eval(const IloAlgorithm alg) const;
	virtual IloExtractableI* makeClone(IloEnvI*) const;
	IloSymbolExprI* getExpr() const { return _arg; }
};

class IloItoaI : public IloSymbolExprI {
	ILOEXTRDECL
		IloIntExprI* _arg;
	virtual void visitSubExtractables(IloExtractableVisitor* v);
public:
	IloItoaI(IloEnvI* env, IloIntExprI* arg) :
		IloSymbolExprI(env), _arg((IloIntExprI*)arg->lockExpr()) {}
	virtual ~IloItoaI();
	void display(ILOSTD(ostream)& out) const;
	virtual IloExtractableI* makeClone(IloEnvI*) const;
