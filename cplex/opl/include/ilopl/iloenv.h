...
class IloCacheGenAlloc : public IloDestroyableI {
	ILORTTIDECL
private:
	IloAllocationStack* _stack;
public:
	IloCacheGenAlloc(IloEnvI* e);
	~IloCacheGenAlloc();
	void * alloc(size_t size) const;
	void free(void * ptr, size_t size) const;
	void reset();
};
inline void * operator new (size_t sz, const IloCacheGenAlloc * gen) {
	return gen->alloc(sz);
}
#ifdef ILODELETEOPERATOR
inline void operator delete(void*, const IloCacheGenAlloc*) { }
#endif
