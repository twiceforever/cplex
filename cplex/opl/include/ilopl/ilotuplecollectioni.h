...
protected:
	...
	IloAnyFixedArray _pointers;
	IloBool _hasReference;
	IloIntFixedArray _referencedColumns;
	IloIntFixedArray _referenceKeyColumns;
...
public:
    IloTupleBufferI* getOrMakeSharedTupleBuffer(){
        IloTupleBufferI* b = getSchema().getImpl()->getOrMakeSharedTupleBuffer();
		b->setCollection(this, IloFalse);
		return b;
    }
	IloTupleCellArray getOrMakeEmptyCells();
	virtual void makeInt2Cells();
	IloArrayI* getInt2Cells() const { return _int2Cells; }
	IloCacheGenAlloc* getAllocator();

	void registerTuple(IloTuple t);
	void unregisterTuple(IloTuple t);
...
public:
	...
	IloIntFixedArray getReferencedColumns() const{
	...
	IloIntFixedArray getReferenceKeyIndexes() const{
	...
	IloAnyFixedArray makePointersToColumns() const;
	IloIntArray getCollectionColumnIndexes() const{ return getSchema().getImpl()->getCollectionColumnIndexes();}
	...
	IloBool hasHasForTupleCollection() const { return _hashForTupleCollection != 0; }
	...
	IloAnyFixedArray getSharedPointersToColumns() const{ return _pointers;}
...
public:
	...
	virtual IloBool isKeyEnabled() const {
		return IloFalse;
	}

protected:
	IloTupleCellArray makeKeyCells(IloInt line, IloMemoryManagerI* mem = 0);
	IloTupleCellArray makeKeyCells(IloTupleCellArray, IloMemoryManagerI* mem = 0);
public:
...
//ok with references.
	inline IloTupleCellArray getOrMakeEmptySharedTupleCells() {
		if (!_sharedTupleCells.getImpl()){
			_sharedTupleCells = IloTupleCellArray(getEnv()->getMemoryManager(), getSchema().getImpl()->getTotalColumnNumber());
	...
	inline IloTupleCellArray getOrMakeEmptySharedKeyCells() {
		if (!_sharedKeyCells.getImpl()){
			_sharedKeyCells = IloTupleCellArray(getEnv()->getMemoryManager(), getSchema().getImpl()->getOrMakeTotalKeyIndexes().getSize());
			return _sharedKeyCells;
		}
	...
	inline IloTupleCellArray getOrMakeEmptyRefSliceCells() {
		if (!_sharedRefSliceCells.getImpl())
			_sharedRefSliceCells = IloTupleCellArray(getEnv()->getMemoryManager());
	...
	inline IloTupleCellArray getOrMakeEmptyReferenceCells() {
		if (!_sharedRefCells.getImpl()){
			...
			_sharedRefCells = IloTupleCellArray(getEnv()->getMemoryManager(), size);
		}
...
public:
	...
	IloDiscreteDataCollection getColumn(IloInt index) const{
		IloTestAndRaise(index != -1, "can not get negative column of tupleSet");
...
public:
	...
	virtual void clear();
	IloAnyArray getAccessors() const { return _tuples; }
	virtual void garbage();
	virtual IloTupleCollectionI* cleanMe();
};
...
protected:
	//IloAllocationStack* _cellsStack;
	//IloMemoryManagerI* _cellsManager;
	IloVBHash* _VBhash;
	IloVBHash* _UnaryVBhash;
	IloTupleCells2IndexHash* _tupleHash;

	IloMemoryManagerI* getCellsMemManager();
private:
	IloDiscreteDataCollectionI* findColumn(const char* name) const;
...
public:
	...
	void clearHashForSelect(){
		...
		if (_UnaryVBhash && _UnaryVBhash->getSize()){
