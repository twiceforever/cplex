...
protected:
    virtual std::ostream& getOut() const;
...
public:
    ...
    virtual std::istream* doResolveStream(const char* basePath, const char* name);
...
public:
    ...
    IloBool isDistributedMIP() const{
        return impl().isDistributedMIP();
	}
    
	const char* getDistributedMIPConfig() const{
        return impl().getDistributedMIPConfig();
	}
    
	void setDistributedMIPConfig(const char* path){
        impl().setDistributedMIPConfig(path);
	}
